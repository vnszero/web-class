...
let PrevButtonEl = document.querySelector('#anterior');
let NextButtonEl = document.querySelector('#proximo');

PrevButtonEl.addEventListener('click', function(){
  let imgEl = document.querySelector('#slide');
  let currentFileName = imgEl.src;
  let lastName = currentFileName.split("/").pop(); 
  let currentNumber = parseInt(lastName.split("-")[0]) - 1;
  if(currentNumber < 1)
  {
    currentNumber = 8;
  }
  imgEl.src = servidorDasImagens + '/' + imagens[parseInt(currentNumber-1)].arquivo;
  imgEl.alt = imagens[currentNumber + 1].descricao;
})
...
NextButtonEl.addEventListener('click', function(){
  ...
  imgEl.src = servidorDasImagens + '/' + imagens[currentNumber + 1].arquivo;
  imgEl.alt = imagens[currentNumber + 1].descricao;
})

// imgEl.src = servidorDasImagens + imagens[0].arquivo;
